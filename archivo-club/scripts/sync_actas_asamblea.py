#!/usr/bin/env python3
"""
Sync assembly-related PDFs from ferrocarriloeste.org.ar into archivo-club/actas-asamblea/.
...
DEFAULT_OUTDIR = REPO_ROOT / "actas-asamblea"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Directory that will receive the PDFs and generated index. Relative paths are resolved under archivo-club/.",
    )
    parser.add_argument(
        "--category-id",
        type=int,
...
def resolve_outdir(raw_outdir: str | Path) -> Path:
    candidate = Path(raw_outdir).expanduser()
    if candidate.is_absolute():
        resolved = candidate.resolve()
    else:
        if candidate.parts and candidate.parts[0] == REPO_ROOT.name:
    ...
    try:
        ...
        raise SystemExit(
            f"--outdir must stay inside {REPO_ROOT}; got {resolved}"
        ) from exc
